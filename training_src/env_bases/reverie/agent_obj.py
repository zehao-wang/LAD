...
class GMapObjectNavAgent(Seq2SeqAgent):
    ...
    def _panorama_feature_variable(self, obs):
        ...
        for i, ob in enumerate(obs):
            ...
            for j, cc in enumerate(ob['candidate']):
                # add candidated navigable point view features
    ...
    def _nav_gmap_variable(self, obs, gmaps):
        ...
        for i, gmap in enumerate(gmaps):
            ...
            for k in gmap.node_positions.keys(): # node_positions={viewpointId : position}
                if gmap.graph.visited(k):
                    visited_vpids.append(k)
                else:
                    unvisited_vpids.append(k) # -> unexplore nodes (eg: candidated nodes)
                
            batch_no_vp_left.append(len(unvisited_vpids) == 0) # Need check
            ...
            )   # cuda  add zero embedding at the first position
            
            
            gmap_pos_fts = gmap.get_pos_fts(
                obs[i]['viewpoint'], gmap_vpids, obs[i]['heading'], obs[i]['elevation'],
            )  # all nodes' position embedding relate to current position 
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        self._update_scanvp_cands(obs) # get candidate point of current position(obs)
        batch_size = len(obs)
        ...
        for i, ob in enumerate(obs):
            ...
            # add graph,node position -> manitan the shortest path graph
        ...
        for t in range(self.args.max_action_len):
            ...
            # transformer_encoder (cat([view_feats, obj_feats]))

            avg_pano_embeds = torch.sum(pano_embeds * pano_masks.unsqueeze(2), 1) / \
                              torch.sum(pano_masks, 1, keepdim=True)
            # average pano embeds (view emb, obj emb )
            # pano_embeds of current node  -> batch_size => diffenct episodes => different graph

            for i, gmap in enumerate(gmaps):
                if not ended[i]:
                    # update visited node -> average pano embeds <- rewrite embeding of visited node
                    i_vp = obs[i]['viewpoint']
                    gmap.update_node_embed(i_vp, avg_pano_embeds[i], rewrite=True)
                    
                    # update unvisited nodes       candidate point img feature
                    for j, i_cand_vp in enumerate(pano_inputs['cand_vpids'][i]):
                        # if candidate point is not visited update that point feature
                        if not gmap.graph.visited(i_cand_vp):
                            gmap.update_node_embed(i_cand_vp, pano_embeds[i, j])
                            # not rewrite -> average embedding from difference view
            ...
            # nav_outs => {'gmap_embeds' ; 'vp_embeds'; 'global_logits' ;}
            #             {'local_logits' ; 'fused_logits' ; 'obj_logits'}
