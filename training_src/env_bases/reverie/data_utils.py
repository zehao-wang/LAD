    ...
    for split in splits:
        if "/" not in split:    # the official splits
            if tokenizer == 'bert':
                ...
                filepath = os.path.join(anno_dir, 'REVERIE_%s_clip.json' % split)
            else:
...
def load_vp2roomlabel(vp2room_file):
    vp2rooms = {}
    with open(vp2room_file,"r") as f:
        data = json.load(f)

    vp2rooms["label_info"] = data["label_info"]
    vp2rooms["vp2room"] = {}
    for scan, vp_infos in data["pano_info"].items():
        for vp, info in vp_infos.items():
            vp2rooms["vp2room"][scan+"_"+vp] = {
                "label":info["label"],
                "id": info["region_label_index"],
